...
function FilterDesktop({ paramss }) {
    ...
    useEffect(() => {
        const getFeature = async () => {
            ...
            setAll({ events, genre, features, types, platform, subscriptions })
        }
        getFeature()
    }, [])
    ...
    const getFilterUrl = (currentSelect) => {
                            ...
                            <div className='w-[90%] mx-auto'>
                                <div className={`${open === filter.id ? "block" : "hidden"}`}>
                                    {filter.array.map((item) => (
                                        <div key={item.id} className="py-2 flex items-center gap-2 text-sm text-[#A7A7A9]">
                                            <input
